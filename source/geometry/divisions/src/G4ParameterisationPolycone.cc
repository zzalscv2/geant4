...
//-----------------------------------------------------------------------
G4VParameterisationPolycone::G4VParameterisationPolycone(EAxis axis, G4int nDiv, G4double width,
  ...
  : G4VDivisionParameterisation(axis, nDiv, width, offset, divType, msolid)
{
  ...
  if (msolid->GetEntityType() == "G4ReflectedSolid")
  {
    ...
    auto newSolid =
      new G4Polycone(msol->GetName(), msol->GetStartPhi(), msol->GetEndPhi() - msol->GetStartPhi(),
...
//---------------------------------------------------------------------
G4ParameterisationPolyconeRho::G4ParameterisationPolyconeRho(EAxis axis, G4int nDiv, G4double width,
                                                             G4double offset, G4VSolid* msolid,
                                                             DivisionType divType)
  : G4VParameterisationPolycone(axis, nDiv, width, offset, msolid, divType)
{
  ...
  if (divType == DivWIDTH)
  {
    fnDiv = CalculateNDiv(origparamMother->Rmax[0] - origparamMother->Rmin[0], width, offset);
  }
  else if (divType == DivNDIV)
  {
    fwidth = CalculateWidth(origparamMother->Rmax[0] - origparamMother->Rmin[0], nDiv, offset);
  }

#ifdef G4DIVDEBUG
  if (verbose >= -1)
  {
    G4cout << " G4ParameterisationPolyconeRho - # divisions " << fnDiv << " = " << nDiv << G4endl
...
void G4ParameterisationPolyconeRho::CheckParametersValidity()
{
  ...
  if (fDivisionType == DivNDIVandWIDTH || fDivisionType == DivWIDTH)
  {
    ...
    G4Exception("G4VParameterisationPolycone::CheckParametersValidity()", "GeomDiv1001",
                JustWarning, message);
  }
  if (foffset != 0.)
  {
...
//---------------------------------------------------------------------
void G4ParameterisationPolyconeRho::ComputeTransformation(const G4int,
                                                          G4VPhysicalVolume* physVol) const
{
  ...
  if (verbose >= 2)
  {
    G4cout << " G4ParameterisationPolyconeRho " << G4endl << " foffset: " << foffset
  ...
  if (verbose >= 2)
  {
    G4cout << std::setprecision(8) << " G4ParameterisationPolyconeRho " << G4endl
           << " Position: (0,0,0)"
           << " - Width: " << fwidth / CLHEP::deg << " - Axis: " << faxis << G4endl;
  }
#endif
}

//---------------------------------------------------------------------
void G4ParameterisationPolyconeRho::ComputeDimensions(G4Polycone& pcone, const G4int copyNo,
...
//---------------------------------------------------------------------
G4ParameterisationPolyconePhi::G4ParameterisationPolyconePhi(EAxis axis, G4int nDiv, G4double width,
                                                             G4double offset, G4VSolid* msolid,
                                                             DivisionType divType)
  : G4VParameterisationPolycone(axis, nDiv, width, offset, msolid, divType)
{
  ...
  if (verbose >= 1)
  {
    G4cout << " G4ParameterisationPolyconePhi - # divisions " << fnDiv << " = " << nDiv << G4endl
                                                          ...
                                                          G4VPhysicalVolume* physVol) const
{
  ...
  if (verbose >= 2)
  {
    G4cout << " G4ParameterisationPolyconePhi - position: " << posi / CLHEP::deg << G4endl
  ...
  if (verbose >= 2)
  {
    G4cout << std::setprecision(8) << " G4ParameterisationPolyconePhi " << copyNo << G4endl
...
//---------------------------------------------------------------------
void G4ParameterisationPolyconePhi::ComputeDimensions(G4Polycone& pcone, const G4int,
                                                      const G4VPhysicalVolume*) const
{
  auto msol = (G4Polycone*)(fmotherSolid);
...
//---------------------------------------------------------------------
G4ParameterisationPolyconeZ::G4ParameterisationPolyconeZ(EAxis axis, G4int nDiv, G4double width,
                                                         G4double offset, G4VSolid* msolid,
                                                         DivisionType divType)
  : G4VParameterisationPolycone(axis, nDiv, width, offset, msolid, divType),
    fOrigParamMother(((G4Polycone*)fmotherSolid)->GetOriginalParameters())
{
  ...
  if (verbose >= 1)
  {
    G4cout << " G4ParameterisationPolyconeZ - # divisions " << fnDiv << " = " << nDiv << G4endl
           << " Offset " << foffset << " = " << offset << G4endl << " Width " << fwidth << " = "
           << width << G4endl;
  }
#endif
...
//------------------------------------------------------------------------
G4double G4ParameterisationPolyconeZ::GetR(G4double z, G4double z1, G4double r1, G4double z2,
                                           G4double r2) const
{
...
void G4ParameterisationPolyconeZ::CheckParametersValidity()
{
  ...
  if (fDivisionType == DivNDIV)
  {
    if (fnDiv > fOrigParamMother->Num_z_planes - 1)
    {
      ...
      error << "Configuration not supported." << G4endl
            << "Division along Z will be done by splitting in the defined" << G4endl
            << "Z planes, i.e, the number of division would be: "
            << fOrigParamMother->Num_z_planes - 1 << ", instead of: " << fnDiv << " !";
      G4Exception("G4ParameterisationPolyconeZ::CheckParametersValidity()", "GeomDiv0001",
  ...
  if (fDivisionType == DivNDIVandWIDTH || fDivisionType == DivWIDTH)
  {
    ...
    {
      std::ostringstream message;
      message << "Condiguration not supported." << G4endl << "Division with user defined width."
                                                        ...
                                                        G4VPhysicalVolume* physVol) const
{
  ...
  if (verbose >= 2)
  {
    G4cout << " G4ParameterisationPolyconeZ - position: " << posi << G4endl << " copyNo: " << copyNo
  ...
  if (verbose >= 2)
  {
    G4cout << std::setprecision(8) << " G4ParameterisationPolyconeZ " << copyNo << G4endl
           << " Position: (0,0,0) - Width: " << fwidth << " - Axis: " << faxis << G4endl;
  }
#endif
}
...
//---------------------------------------------------------------------
void G4ParameterisationPolyconeZ::ComputeDimensions(G4Polycone& pcone, const G4int copyNo,
                                                    const G4VPhysicalVolume*) const
{
