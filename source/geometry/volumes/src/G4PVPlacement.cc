...
namespace
{
class ScopedOverlapRandomEngine
{
  public:

    explicit ScopedOverlapRandomEngine(std::uint64_t seed)
    {
      if (seed == 0)
      {
        return;
      ...
      fPrevious = G4Random::getTheEngine();
      auto engineSeed = static_cast<long>(seed & 0x7fffffffU);
      if (engineSeed == 0)
      {
        engineSeed = 1;
      }
      fEngine = std::make_unique<CLHEP::HepJamesRandom>(engineSeed);
      G4Random::setTheEngine(fEngine.get());
      auto quickSeed = static_cast<std::uint32_t>(seed);
      G4QuickRand((quickSeed == 0) ? 1 : quickSeed);
    }

    ~ScopedOverlapRandomEngine()
    {
      if (fPrevious != nullptr)
      {
        G4Random::setTheEngine(fPrevious);
      }
    }

    ScopedOverlapRandomEngine(const ScopedOverlapRandomEngine&) = delete;
    ScopedOverlapRandomEngine& operator=(const ScopedOverlapRandomEngine&) = delete;

  private:

    std::unique_ptr<CLHEP::HepJamesRandom> fEngine;
    CLHEP::HepRandomEngine* fPrevious = nullptr;
};
}  // namespace
...
// Constructor
//
G4PVPlacement::G4PVPlacement(G4RotationMatrix* pRot, const G4ThreeVector& tlate,
                             const G4String& pName, G4LogicalVolume* pLogical,
                             G4VPhysicalVolume* pMother, G4bool pMany, G4int pCopyNo,
                             G4bool pSurfChk)
  : G4VPhysicalVolume(pRot, tlate, pName, pLogical, pMother), fmany(pMany), fcopyNo(pCopyNo)
{
  if (pMother != nullptr)
  {
    ...
    {
      G4Exception("G4PVPlacement::G4PVPlacement()", "GeomVol0002", FatalException,
...
// Constructor
//
G4PVPlacement::G4PVPlacement(const G4Transform3D& Transform3D, const G4String& pName,
                             G4LogicalVolume* pLogical, G4VPhysicalVolume* pMother, G4bool pMany,
                             G4int pCopyNo, G4bool pSurfChk)
...
G4PVPlacement::G4PVPlacement(G4RotationMatrix* pRot, const G4ThreeVector& tlate,
                             ...
                             G4bool pSurfChk)
  : G4VPhysicalVolume(pRot, tlate, pName, pCurrentLogical, nullptr), fmany(pMany), fcopyNo(pCopyNo)
{
  ...
  if ((pSurfChk) && ((pMotherLogical) != nullptr))
  {
    CheckOverlaps();
  }
}
...
// Constructor
//
G4PVPlacement::G4PVPlacement(const G4Transform3D& Transform3D, G4LogicalVolume* pCurrentLogical,
                             const G4String& pName, G4LogicalVolume* pMotherLogical, G4bool pMany,
                             G4int pCopyNo, G4bool pSurfChk)
  : G4VPhysicalVolume(nullptr, Transform3D.getTranslation(), pName, pCurrentLogical, nullptr),
    fmany(pMany),
    fcopyNo(pCopyNo)
{
  if (pCurrentLogical == pMotherLogical)
...
//
G4PVPlacement::G4PVPlacement(__void__& a) : G4VPhysicalVolume(a) {}
...
// GetReplicationData
//
void G4PVPlacement::GetReplicationData(EAxis&, G4int&, G4double&, G4double&, G4bool&) const
{
  // No-operations
}
...
// CheckOverlaps
//
G4bool G4PVPlacement::CheckOverlaps(G4int res, G4double tol, G4bool verbose, G4int maxErr)
{
  G4bool checked = false;
  G4bool completed = true;
  std::vector<G4String> warnings;
  G4bool overlap = ComputeOverlaps(res, tol, maxErr, 0, checked, completed, warnings);
  ReportOverlaps(overlap, checked, completed, warnings, verbose);
  return overlap;
}
...
// ComputeOverlaps
//
G4bool G4PVPlacement::ComputeOverlaps(G4int res, G4double tol, G4int maxErr, std::uint64_t seed,
                                      G4bool& checked, G4bool& completed,
                                      std::vector<G4String>& warnings)
{
  ...
  ScopedOverlapRandomEngine randomEngine(seed);
  G4int trials = 0;

  // Check that random points are generated correctly
  //
  ...
  if (solid->Inside(ptmp) != kSurface)
  {
    ...
    message << "Sample point is not on the surface !" << G4endl
            << "          The issue is detected for volume " << GetName() << ':' << GetCopyNo()
            << " (" << solid->GetEntityType() << ")" << G4endl << "          generated point "
            << ptmp << " is " << position[solid->Inside(ptmp)];
  ...
  if (overlapCount > 0)
  {
    ...
    message << "Overlap with mother volume !" << G4endl
            << "          Overlap is detected for volume " << GetName() << ':' << GetCopyNo()
            << " (" << solid->GetEntityType() << ")"
            << " with its mother volume " << motherLog->GetName() << " ("
            << motherSolid->GetEntityType() << ")" << G4endl
            << "          protrusion at mother local point " << overlapPoint << " by "
            << G4BestUnit(overlapSize, "Length") << " (max of " << overlapCount << " cases)";
    if (trials >= maxErr)
    {
  ...
  for (std::size_t k = 0; k < motherLog->GetNoDaughters(); ++k)
  {
    ...
    else  // transformation with rotation
    {
      ...
      if ((scenter - dcenter).mag2() >= (sradius + dradius) * (sradius + dradius))
      {
        continue;
      }
      if (dcenter.x() - dradius >= xmax)
      {
        continue;
      }
      if (dcenter.y() - dradius >= ymax)
      {
        continue;
      }
      if (dcenter.z() - dradius >= zmax)
      {
        continue;
      }
      if (dcenter.x() + dradius <= xmin)
      {
        continue;
      }
      if (dcenter.y() + dradius <= ymin)
      {
        continue;
      }
      if (dcenter.z() + dradius <= zmin)
      {
        continue;
      }

      G4ThreeVector pbox[8] = {
        G4ThreeVector(pmin.x(), pmin.y(), pmin.z()), G4ThreeVector(pmax.x(), pmin.y(), pmin.z()),
        G4ThreeVector(pmin.x(), pmax.y(), pmin.z()), G4ThreeVector(pmax.x(), pmax.y(), pmin.z()),
        G4ThreeVector(pmin.x(), pmin.y(), pmax.z()), G4ThreeVector(pmax.x(), pmin.y(), pmax.z()),
        G4ThreeVector(pmin.x(), pmax.y(), pmax.z()), G4ThreeVector(pmax.x(), pmax.y(), pmax.z())};
    ...
    if (overlapCount > 0)
    {
      ...
      message << "Overlap with volume already placed !" << G4endl
              << "          Overlap is detected for volume " << GetName() << ':' << GetCopyNo()
              << " (" << solid->GetEntityType() << ") with " << daughter->GetName() << ':'
              << daughter->GetCopyNo() << " (" << daughterSolid->GetEntityType() << ")" << G4endl
              << "          overlap at local point " << overlapPoint << " by "
              << G4BestUnit(overlapSize, "Length") << " (max of " << overlapCount << " cases)";
      if (trials >= maxErr)
      {
        message << G4endl << "NOTE: Reached maximum fixed number -" << maxErr
    ...
    else if (check_encapsulation)
    {
      ...
      G4ThreeVector dPoint = (daughterSolid->Inside(pInside) == kInside) ? pInside : pSurface;
      ...
      if (solid->Inside(msi) == kInside)
      {
        ...
        message << "Overlap with volume already placed !" << G4endl
                ...
                << "          apparently fully encapsulating volume " << daughter->GetName() << ':'
                << daughter->GetCopyNo() << " (" << daughterSolid->GetEntityType() << ")"
                << " at the same level!";
        if (trials >= maxErr)
        {
          message << G4endl << "NOTE: Reached maximum fixed number -" << maxErr
...
// ReportOverlaps
//
void G4PVPlacement::ReportOverlaps(G4bool overlap, G4bool checked, G4bool completed,
                                   const std::vector<G4String>& warnings, G4bool verbose) const
{
  ...
  if (verbose)
  {
    G4cout << "Checking overlaps for volume " << GetName() << ':' << GetCopyNo() << " ("
           << solid->GetEntityType() << ") ... ";
  }

  for (const auto& message : warnings)
  {
    G4Exception("G4PVPlacement::CheckOverlaps()", "GeomVol1002", JustWarning, message.c_str());
  }

  if (verbose && completed && !overlap)
  {
    G4cout << "OK! " << G4endl;
  }
}
