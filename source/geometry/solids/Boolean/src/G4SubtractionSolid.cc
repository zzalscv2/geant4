...
G4SubtractionSolid::G4SubtractionSolid(const G4String& pName, G4VSolid* pSolidA, G4VSolid* pSolidB)
...
G4SubtractionSolid::G4SubtractionSolid(__void__& a) : G4BooleanSolid(a) {}
...
void G4SubtractionSolid::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4SubtractionSolid::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
G4bool G4SubtractionSolid::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
                                           ...
                                           G4double& pMax) const
{
  ...
  return fPtrSolidA->CalculateExtent(pAxis, pVoxelLimit, pTransform, pMin, pMax);
}
...
EInside G4SubtractionSolid::Inside(const G4ThreeVector& p) const
{
  ...
  return ((fPtrSolidA->SurfaceNormal(p) - fPtrSolidB->SurfaceNormal(p)).mag2() > rtol) ? kSurface
                                                                                       : kOutside;
}
...
G4ThreeVector G4SubtractionSolid::SurfaceNormal(const G4ThreeVector& p) const
{
  ...
  if (InsideA == kOutside)
  {
    ...
    G4cout << "WARNING - Invalid call [1] in "
           << "G4SubtractionSolid::SurfaceNormal(p)" << G4endl << "  Point p is outside !"
           << G4endl;
    G4cout << "          p = " << p << G4endl;
  ...
  else if (InsideA == kSurface && InsideB != kInside)
  {
    normal = fPtrSolidA->SurfaceNormal(p);
  }
  else if (InsideA == kInside && InsideB != kOutside)
  {
  ...
  else
  {
    ...
    if (Inside(p) == kInside)
    {
      G4cout << "WARNING - Invalid call [2] in "
             << "G4SubtractionSolid::SurfaceNormal(p)" << G4endl << "  Point p is inside !"
             << G4endl;
      G4cout << "          p = " << p << G4endl;
...
G4double G4SubtractionSolid::DistanceToIn(const G4ThreeVector& p, const G4ThreeVector& v) const
{
  ...
  if (Inside(p) == kInside)
  {
    G4cout << "WARNING - Invalid call in "
           << "G4SubtractionSolid::DistanceToIn(p,v)" << G4endl << "  Point p is inside !"
           << G4endl;
    G4cout << "          p = " << p << G4endl;
    G4cout << "          v = " << v << G4endl;
      ...
      do  // Loop checking, 13.08.2015, G.Cosmo
      {
        ...
        if (Inside(p + dist * v) == kOutside)
        {
          ...
          if (count1 > 1000)  // Infinite loop detected
          {
            ...
            std::ostringstream message;
            message << "Illegal condition caused by solids: " << fPtrSolidA->GetName() << " and "
                    << nameB << G4endl;
            message.precision(16);
            message << "Looping detected in point " << p + dist * v << ", from original point " << p
                    << " and direction " << v << G4endl << "Computed candidate distance: " << dist
                    << "*mm. ";
            message.precision(6);
            DumpInfo();
            G4Exception("G4SubtractionSolid::DistanceToIn(p,v)", "GeomSolids1001", JustWarning,
                        message, "Returning candidate distance.");
            return dist;
          }
        }
    ...
    while (Inside(p + dist * v) == kOutside)  // pushing loop
    {
      ...
      if (Inside(p + dist * v) == kOutside)
      {
        ...
        if (count2 > 1000)  // Infinite loop detected
        {
          ...
          {
            nameB =
              (dynamic_cast<G4DisplacedSolid*>(fPtrSolidB))->GetConstituentMovedSolid()->GetName();
          }
          std::ostringstream message;
          message << "Illegal condition caused by solids: " << fPtrSolidA->GetName() << " and "
                  << nameB << G4endl;
...
G4double G4SubtractionSolid::DistanceToIn(const G4ThreeVector& p) const
{
  ...
  {
    G4cout << "WARNING - Invalid call in "
           << "G4SubtractionSolid::DistanceToIn(p)" << G4endl << "  Point p is inside !" << G4endl;
    G4cout << "          p = " << p << G4endl;
    G4cerr << "WARNING - Invalid call in "
           << "G4SubtractionSolid::DistanceToIn(p)" << G4endl << "  Point p is inside !" << G4endl;
    G4cerr << "          p = " << p << G4endl;
...
G4double G4SubtractionSolid::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
                                           ...
                                           G4ThreeVector* n) const
{
  ...
  if (Inside(p) == kOutside)
  {
    ...
    G4cout << "WARNING - Invalid call in "
           << "G4SubtractionSolid::DistanceToOut(p,v)" << G4endl << "  Point p is outside !"
           << G4endl;
    G4cout << "          p = " << p << G4endl;
    G4cout << "          v = " << v << G4endl;
    G4cerr << "WARNING - Invalid call in "
...
G4double G4SubtractionSolid::DistanceToOut(const G4ThreeVector& p) const
{
...
#ifdef G4BOOLDEBUG
    G4cout << "WARNING - Invalid call in "
           << "G4SubtractionSolid::DistanceToOut(p)" << G4endl << "  Point p is outside" << G4endl;
    G4cout << "          p = " << p << G4endl;
    G4cerr << "WARNING - Invalid call in "
           << "G4SubtractionSolid::DistanceToOut(p)" << G4endl << "  Point p is outside" << G4endl;
    G4cerr << "          p = " << p << G4endl;
  ...
  {
    dist = std::min(fPtrSolidA->DistanceToOut(p), fPtrSolidB->DistanceToIn(p));
  }
  return dist;
}
...
G4double G4SubtractionSolid::GetCubicVolume()
{
  ...
  else
  {
    ...
    {
      G4IntersectionSolid intersectVol("Temporary-Intersection-for-Subtraction", fPtrSolidA,
                                       fPtrSolidB);
