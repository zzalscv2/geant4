...
G4UnionSolid::G4UnionSolid(const G4String& pName, G4VSolid* pSolidA, G4VSolid* pSolidB)
...
G4UnionSolid::G4UnionSolid(__void__& a) : G4BooleanSolid(a) {}

//////////////////////////////////////////////////////////////////////////
//
// Copy constructor

G4UnionSolid::G4UnionSolid(const G4UnionSolid& rhs) : G4BooleanSolid(rhs)
{
  fPMin = rhs.fPMin;
  fPMax = rhs.fPMax;
...
void G4UnionSolid::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  pMin.set(std::min(minA.x(), minB.x()), std::min(minA.y(), minB.y()),
           std::min(minA.z(), minB.z()));

  pMax.set(std::max(maxA.x(), maxB.x()), std::max(maxA.y(), maxB.y()),
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4UnionSolid::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
G4bool G4UnionSolid::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
                                     const G4AffineTransform& pTransform, G4double& pMin,
                                     G4double& pMax) const
{
  G4bool touchesA, touchesB, out;
  G4double minA = kInfinity, minB = kInfinity, maxA = -kInfinity, maxB = -kInfinity;
...
EInside G4UnionSolid::Inside(const G4ThreeVector& p) const
{
  ...
  //
  static const G4double rtol = 1000 * G4GeometryTolerance::GetInstance()->GetRadialTolerance();

  return ((fPtrSolidA->SurfaceNormal(p) + fPtrSolidB->SurfaceNormal(p)).mag2() < rtol) ? kInside
                                                                                       : kSurface;
}
...
G4ThreeVector G4UnionSolid::SurfaceNormal(const G4ThreeVector& p) const
{
  ...
  message << "Invalid call of SurfaceNormal(p) for union solid: " << GetName() << " !"
          << "\nPoint p" << p << " is " << surf[Inside(p)] << " !!!";
  message.precision(oldprc);
  G4Exception("G4UnionSolid::SurfaceNormal()", "GeomMgt0001", JustWarning, message);
#endif
  return fPtrSolidA->SurfaceNormal(p);
}
...
G4double G4UnionSolid::DistanceToIn(const G4ThreeVector& p, const G4ThreeVector& v) const
{
  ...
  if (Inside(p) == kInside)
  {
    G4cout << "WARNING - Invalid call in "
           << "G4UnionSolid::DistanceToIn(p,v)" << G4endl << "  Point p is inside !" << G4endl;
    G4cout << "          p = " << p << G4endl;
    G4cout << "          v = " << v << G4endl;
  ...
  return std::min(fPtrSolidA->DistanceToIn(p, v), fPtrSolidB->DistanceToIn(p, v));
}
...
G4double G4UnionSolid::DistanceToIn(const G4ThreeVector& p) const
{
  ...
  {
    G4cout << "WARNING - Invalid call in "
           << "G4UnionSolid::DistanceToIn(p)" << G4endl << "  Point p is inside !" << G4endl;
    G4cout << "          p = " << p << G4endl;
    G4cerr << "WARNING - Invalid call in "
           << "G4UnionSolid::DistanceToIn(p)" << G4endl << "  Point p is inside !" << G4endl;
    G4cerr << "          p = " << p << G4endl;
...
G4double G4UnionSolid::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
                                     ...
                                     G4ThreeVector* n) const
{
  ...
  if (Inside(p) == kOutside)
  {
    ...
    G4cout << "WARNING - Invalid call in "
           << "G4UnionSolid::DistanceToOut(p,v)" << G4endl << "  Point p is outside !" << G4endl;
    G4cout << "          p = " << p << G4endl;
    G4cout << "          v = " << v << G4endl;
    G4cerr << "WARNING - Invalid call in "
           << "G4UnionSolid::DistanceToOut(p,v)" << G4endl << "  Point p is outside !" << G4endl;
  ...
  else
  {
    ...
    if (positionA != kOutside)
    {
      ...
      {
        disTmp = fPtrSolidA->DistanceToOut(p + dist * v, v, calcNorm, validNorm, nTmp);
        dist += disTmp;

        if (fPtrSolidB->Inside(p + dist * v) != kOutside)
        {
          disTmp = fPtrSolidB->DistanceToOut(p + dist * v, v, calcNorm, validNorm, nTmp);
          dist += disTmp;
        }
      } while ((fPtrSolidA->Inside(p + dist * v) != kOutside) && (disTmp > halfCarTolerance));
    }
    else  // if( positionB != kOutside )
    {
      ...
      } while ((fPtrSolidB->Inside(p + dist * v) != kOutside) && (disTmp > halfCarTolerance));
    }
  }
  if (calcNorm)
  {
...
G4double G4UnionSolid::DistanceToOut(const G4ThreeVector& p) const
{
...
#ifdef G4BOOLDEBUG
    G4cout << "WARNING - Invalid call in "
           << "G4UnionSolid::DistanceToOut(p)" << G4endl << "  Point p is outside !" << G4endl;
    G4cout << "          p = " << p << G4endl;
    G4cerr << "WARNING - Invalid call in "
           << "G4UnionSolid::DistanceToOut(p)" << G4endl << "  Point p is outside !" << G4endl;
    G4cerr << "          p = " << p << G4endl;
  ...
  else
  {
    ...
    {
      distout = std::max(fPtrSolidA->DistanceToOut(p), fPtrSolidB->DistanceToOut(p));
    }
    else
    {
      if (positionA == kOutside)
...
G4double G4UnionSolid::GetCubicVolume()
{
  ...
  G4bool noIntersection = bminA.x() >= bmaxB.x() || bminA.y() >= bmaxB.y() || bminA.z() >= bmaxB.z()
                          || bminB.x() >= bmaxA.x() || bminB.y() >= bmaxA.y()
                          || bminB.z() >= bmaxA.z();
  ...
  else
  {
    ...
    {
      G4IntersectionSolid intersectVol("Temporary-Intersection-for-Union", fPtrSolidA, fPtrSolidB);
      intersectVol.SetCubVolStatistics(GetCubVolStatistics());
      intersectVol.SetCubVolEpsilon(GetCubVolEpsilon());

      fCubicVolume =
        fPtrSolidA->GetCubicVolume() + fPtrSolidB->GetCubicVolume() - intersectVol.GetCubicVolume();
    }
  }
  l.unlock();
  return fCubicVolume;
