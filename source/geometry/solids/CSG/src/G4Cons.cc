...
G4Cons::G4Cons(const G4String& pName, G4double pRmin1, G4double pRmax1, G4double pRmin2,
    ...
    fDPhi(0.)
{
  ...
  if (pDz < 0)
  {
    ...
    message << "Invalid Z half-length for Solid: " << GetName() << G4endl << "        hZ = " << pDz;
    G4Exception("G4Cons::G4Cons()", "GeomSolids0002", FatalException, message);
  }
  ...
  if (((pRmin1 >= pRmax1) || (pRmin2 >= pRmax2) || (pRmin1 < 0)) && (pRmin2 < 0))
  {
    ...
    message << "Invalid values of radii for Solid: " << GetName() << G4endl
            << "        pRmin1 = " << pRmin1 << ", pRmin2 = " << pRmin2 << ", pRmax1 = " << pRmax1
            << ", pRmax2 = " << pRmax2;
    G4Exception("G4Cons::G4Cons()", "GeomSolids0002", FatalException, message);
  }
...
//
G4Cons::G4Cons(__void__& a) : G4CSGSolid(a) {}
...
EInside G4Cons::Inside(const G4ThreeVector& p) const
{
  ...
  if (!fPhiFullCone && ((p.x() != 0.0) || (p.y() != 0.0)))
  {
    ...
    if ((pPhi < fSPhi - halfAngTolerance) || (pPhi > fSPhi + fDPhi + halfAngTolerance))
    {
      return in = kOutside;
    }
...
void G4Cons::ComputeDimensions(G4VPVParameterisation* p, const G4int n,
...
void G4Cons::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (GetDeltaPhiAngle() < twopi)
  {
    ...
    G4GeomTools::DiskExtent(rmin, rmax, GetSinStartPhi(), GetCosStartPhi(), GetSinEndPhi(),
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4Cons::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
G4bool G4Cons::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
G4ThreeVector G4Cons::SurfaceNormal(const G4ThreeVector& p) const
{
  ...
  {
#  ifdef G4CSGDEBUG
    G4Exception("G4Cons::SurfaceNormal(p)", "GeomSolids1002", JustWarning,
...
G4ThreeVector G4Cons::ApproxSurfaceNormal(const G4ThreeVector& p) const
{
  ...
  if (!fPhiFullCone && (rho != 0.0))  // Protected against (0,0,z)
  {
  ...
  switch (side)
  {
    ...
    {
      DumpInfo();
      G4Exception("G4Cons::ApproxSurfaceNormal()", "GeomSolids1002", JustWarning,
...
G4double G4Cons::DistanceToIn(const G4ThreeVector& p, const G4ThreeVector& v) const
{
  ...
  if (std::fabs(nt1) > kRadTolerance)  // Equation quadratic => 2 roots
  {
    ...
    c = nt3 / nt1;
    d = b * b - c;
    if ((nt3 > rout * rout * kRadTolerance * kRadTolerance * secRMax * secRMax) || (rout < 0))
    {
    ...
    else
    {
      ...
      if ((t3 > (rin + halfRadTolerance * secRMin) * (rin + halfRadTolerance * secRMin))
...
G4double G4Cons::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
                               const G4bool calcNorm, G4bool* validNorm, G4ThreeVector* n) const
{
  ...
  if (!fPhiFullCone)
  {
    ...
    else
    {
      ...
      if ((fSPhi - halfAngTolerance <= vphi) && (vphi <= fSPhi + fDPhi + halfAngTolerance))
      {
        sphi = kInfinity;
      }
      else
    ...
    {  // Note: returned vector not normalised
      ...
      default:
        ...
        message << "Undefined side for valid surface normal to solid." << G4endl
                ...
                << G4endl << G4endl
                << "pho at z = " << std::sqrt(p.x() * p.x() + p.y() * p.y()) / mm << " mm" << G4endl
                << G4endl;
        if (p.x() != 0. || p.y() != 0.)
        {
          message << "point phi = " << std::atan2(p.y(), p.x()) / degree << " degrees" << G4endl
                  << G4endl;
        }
        message << "Direction:" << G4endl << G4endl << "v.x() = " << v.x() << G4endl
                << "v.y() = " << v.y() << G4endl << "v.z() = " << v.z() << G4endl << G4endl
                << "Proposed distance :" << G4endl << G4endl << "snxt = " << snxt / mm << " mm"
                << G4endl;
        message.precision(oldprc);
        G4Exception("G4Cons::DistanceToOut(p,v,..)", "GeomSolids1002", JustWarning, message);
        break;
    }
  }
  if (snxt < halfCarTolerance)
...
#  ifdef G4CSGDEBUG
  if (Inside(p) == kOutside)
  {
    ...
    G4cout << "pho at z = " << std::sqrt(p.x() * p.x() + p.y() * p.y()) / mm << " mm" << G4endl
           << G4endl;
    if ((p.x() != 0.) || (p.x() != 0.))
    {
      G4cout << "point phi = " << std::atan2(p.y(), p.x()) / degree << " degrees" << G4endl
             << G4endl;
    }
    G4cout.precision(oldprc);
    G4Exception("G4Cons::DistanceToOut(p)", "GeomSolids1002", JustWarning, "Point p is outside !?");
  }
#  endif
...
G4ThreeVector G4Cons::GetPointOnSurface() const
{
  ...
  if ((chose >= Aone + Atwo + Athree) && (chose < Aone + Atwo + Athree + Afour))  // base at +Dz
  {
...
G4double G4Cons::GetCubicVolume()
{
  if (fCubicVolume == 0)
  {
    ...
    fCubicVolume =
      fDPhi * fDz * (Rmean * Rmean - rMean * rMean + (deltaR * deltaR - deltar * deltar) / 12);
    l.unlock();
  }
  return fCubicVolume;
}
...
G4double G4Cons::GetSurfaceArea()
{
  if (fSurfaceArea == 0)
  {
    ...
    fSurfaceArea =
      fDPhi
      * (mmin * std::sqrt(dmin * dmin + 4 * fDz * fDz)
