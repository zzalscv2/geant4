...
// order in (r,z).
//
G4PolyhedraSide::G4PolyhedraSide(const G4PolyhedraSideRZ* prevRZ, const G4PolyhedraSideRZ* tail,
                                 const G4PolyhedraSideRZ* head, const G4PolyhedraSideRZ* nextRZ,
                                 G4int theNumSide, G4double thePhiStart, G4double thePhiTotal,
                                 G4bool thePhiIsOpen, G4bool isAllBehind)
{
  ...
  G4ThreeVector a1(r[0] * std::cos(phi), r[0] * std::sin(phi), z[0]),
    ...
    d1(nextRZ->r * std::cos(phi), nextRZ->r * std::sin(phi), nextRZ->z), a2, b2, c2, d2;
...
void G4PolyhedraSide::CopyStuff(const G4PolyhedraSide& source)
{
  ...
  G4PolyhedraSideEdge *edge = edges, *sourceEdge = source.edges;
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    *edge = *sourceEdge;
  ...
  G4PolyhedraSideVec *vec = vecs, *sourceVec = source.vecs;
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    *vec = *sourceVec;
...
//
//
G4bool G4PolyhedraSide::Intersect(const G4ThreeVector& p, const G4ThreeVector& v, G4bool outgoing,
                                  ...
                                  G4bool& isAllBehind)
{
  ...
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    if (normSign * qc.cross(qd).dot(v) < 0)
    {
      continue;
    }
    ...
    if (normSign * qa.cross(qb).dot(v) > 0)
    {
      continue;
    }
    ...
    if (r[0] > 1 / kInfinity && normSign * qa.cross(qc).dot(v) < 0)
    {
      return false;
    }
    if (r[1] > 1 / kInfinity && normSign * qb.cross(qd).dot(v) > 0)
    {
      return false;
    }
    ...
    if (distFromSurface < 0)
    {
      ...
      if (std::fabs(rz) > lenRZ + surfTolerance)
      {
        return false;
      }

      G4double pp = ps.dot(vec->surfPhi);
      if (std::fabs(pp) > lenPhi[0] + lenPhi[1] * rz + surfTolerance)
      {
        return false;
      }
    }
...
//
void G4PolyhedraSide::CalculateExtent(const EAxis axis, const G4VoxelLimits& voxelLimit,
                                           ...
                                           G4double& distFromSurface)
{
  ...
  if (atRZ < 0)
  {
    ...
    {
      return false;
    }  // Forget it! Missed by a mile.

    G4ThreeVector q = p + v;
    G4ThreeVector qa = q - vec.edges[0]->corner[0], qb = q - vec.edges[1]->corner[0];
    G4ThreeVector qacb = qa.cross(qb);
    if (normSign * qacb.dot(v) < 0)
    {
      return false;
    }
  ...
  else if (atRZ > 0)
  {
    ...
    G4ThreeVector q = p + v;
    G4ThreeVector qa = q - vec.edges[0]->corner[1], qb = q - vec.edges[1]->corner[1];
    G4ThreeVector qacb = qa.cross(qb);
    if (normSign * qacb.dot(v) >= 0)
    {
      return false;
...
// the segment.
//
G4int G4PolyhedraSide::LineHitsSegments(const G4ThreeVector& p, const G4ThreeVector& v, G4int* i1,
                                        G4int* i2)
{
  G4double s1, s2;
  //
...
//
G4double G4PolyhedraSide::DistanceToOneSide(const G4ThreeVector& p, const G4PolyhedraSideVec& vec,
                                            G4double* normDist)
{
...
//
G4double G4PolyhedraSide::DistanceAway(const G4ThreeVector& p, const G4PolyhedraSideVec& vec,
                                       G4double* normDist)
{
  G4double distOut2;
  G4ThreeVector pct = p - vec.center;
...
//
G4double G4PolyhedraSide::SurfaceTriangle(const G4ThreeVector& p1, const G4ThreeVector& p2,
...
//
G4ThreeVector G4PolyhedraSide::GetPointOnPlane(const G4ThreeVector& p0, const G4ThreeVector& p1,
