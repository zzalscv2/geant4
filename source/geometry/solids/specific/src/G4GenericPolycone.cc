...
G4Mutex genpolyMutex = G4MUTEX_INITIALIZER;
}  // namespace
...
//
G4GenericPolycone::G4GenericPolycone(const G4String& name, G4double phiStart, G4double phiTotal,
                                     G4int numRZ, const G4double r[], const G4double z[])
  : G4VCSGfaceted(name)
{
...
// Create
//
// Generic create routine, called by constructor after conversion of arguments
//
void G4GenericPolycone::Create(G4double phiStart, G4double phiTotal, G4ReduciblePolygon* rz)
{
  ...
  if (rz->Amin() < 0.0)
  {
    ...
    G4Exception("G4GenericPolycone::Create()", "GeomSolids0002", FatalErrorInArgument, message);
  }
...
// Copy constructor
//
G4GenericPolycone::G4GenericPolycone(const G4GenericPolycone& source) : G4VCSGfaceted(source)
{
  CopyStuff(source);
}
...
G4bool G4GenericPolycone::Reset()
{
  ...
  G4Exception("G4GenericPolycone::Reset()", "GeomSolids1001", JustWarning, message,
...
void G4GenericPolycone::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("GenericG4Polycone::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
//
G4bool G4GenericPolycone::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
#  endif
  ...
  if (!G4GeomTools::TriangulatePolygon(contourRZ, triangles))
  {
    ...
    G4Exception("G4GenericPolycone::CalculateExtent()", "GeomMgt1002", JustWarning, message);
    return bbox.CalculateExtent(pAxis, pVoxelLimit, pTransform, pMin, pMax);
  }
...
G4ThreeVector G4GenericPolycone::GetPointOnSurface() const
{
  ...
  auto it = std::lower_bound(fElements->begin(), fElements->end(), select,
                             [](const G4GenericPolycone::surface_element& x,
                                G4double val) -> G4bool { return x.area < val; });
