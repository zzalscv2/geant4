...
//
G4Polycone::G4Polycone(const G4String& name, G4double phiStart, G4double phiTotal, G4int numZPlanes,
...
//
G4Polycone::G4Polycone(const G4String& name, G4double phiStart, G4double phiTotal, G4int numRZ,
...
//
G4Polycone::G4Polycone(__void__& a) : G4VCSGfaceted(a), startPhi(0.), endPhi(0.), numCorner(0) {}
...
void G4Polycone::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4Polycone::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
G4ThreeVector G4Polycone::GetPointOnSurface() const
{
  ...
  auto it = std::lower_bound(
    ...
    [](const G4Polycone::surface_element& x, G4double val) -> G4bool { return x.area < val; });
