...
G4bool G4GeometryManager::BuildOptimisations(G4bool allOpts, G4bool verbose)
{
  ...
  fOptimiseInParallelConfigured =
    fParallelVoxelOptimisationRequested && G4Threading::IsMultithreadedApplication();
...
// ***************************************************************************
//
void G4GeometryManager::BuildOptimisationsSequential(G4bool allOpts, G4bool verbose)
{
  G4Timer timer;
  G4Timer allTimer;
  ...
  G4cout << G4endl << "*** G4GeometryManager::BuildOptimisationsSequential() called on tid "
            ...
            && (volume->GetDaughter(0)->GetRegularStructureId() != 1)))
    {
      ...
      else
      {
        ...
        message << "VoxelHeader allocation error." << G4endl << "Allocation of new VoxelHeader"
                << G4endl << "        for volume '" << volume->GetName() << "' failed.";
        G4Exception("G4GeometryManager::BuildOptimisations()", "GeomMgt0003", FatalException,
                    message);
      }
      if (verbose)
      {
        timer.Stop();
        stats.emplace_back(volume, head, timer.GetSystemElapsed(), timer.GetUserElapsed());
      }
    }
    else
    {
      ...
      G4cout << "- Skipping logical volume with " << numDaughters << " daughters and name = '"
      ...
      else
      {
        if (numDaughters == 1)
        {
          G4cout << (volume->GetDaughter(0)->IsReplicated() ? "[Replicated]" : "[Placement]");
        }
      }
      G4cout << G4endl;
#endif
  ...
  if (verbose)
  {
    ...
    G4VoxelisationHelper::ReportVoxelStats(stats,
                                           allTimer.GetSystemElapsed() + allTimer.GetUserElapsed());
  }
}
...
//
G4bool G4GeometryManager::BuildOptimisations(G4bool allOpts, G4VPhysicalVolume* pVolume)
{
  if (pVolume == nullptr)
  {
    return false;
  ...
  if (((tVolume->IsToOptimise())
      ...
      || ((tVolume->GetNoDaughters() == 1) && (tVolume->GetDaughter(0)->IsReplicated())))
  {
    ...
    else
    {
      std::ostringstream message;
      message << "VoxelHeader allocation error." << G4endl << "Allocation of new VoxelHeader"
              << G4endl << "        for volume " << tVolume->GetName() << " failed.";
      G4Exception("G4GeometryManager::BuildOptimisations()", "GeomMgt0003", FatalException,
                  message);
    }
  }
  else
  {
    ...
    G4cout << "** G4GeometryManager::BuildOptimisations()" << G4endl
           << "     Skipping logical volume name = " << tVolume->GetName() << G4endl;
#endif
  }
...
void G4GeometryManager::SetWorldMaximumExtent(G4double extent)
{
  if (!G4SolidStore::GetInstance()->empty())
  {
    ...
    //
    G4Exception("G4GeometryManager::SetMaximumExtent()", "GeomMgt0003", FatalException,
