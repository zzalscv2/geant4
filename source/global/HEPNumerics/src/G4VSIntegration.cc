...
G4double G4VSIntegration::ComputeIntegral(const G4double emin, const G4double emax)
{
  ...
  for (G4int i = 0; i <= nbin; ++i)
  {
    ...
    if ((del < fAcc * res && 0 < fP2) || endpoint)
    {
      break;
    }
    problast = y;
  ...
  {
    G4cout << "### G4VSIntegration::ComputeIntegral: "
           << "I=" << res << " E1=" << fE1 << " E2=" << fE2 << " Pmax=" << fPmax << " P1=" << fP1
           << " P2=" << fP2 << G4endl;
  }
#endif
  return res;
  ...
  for (G4int n = 0; n < nmax; ++n)
  {
    ...
    if ((gg > gmax || n >= nmax) && fVerbose > 0)
    {
      ...
      if (fnWarn < fWarnLimit)
      {
        ...
        G4cout << "### G4VSIntegration::SampleValue() for " << ModelName() << " in area=" << idx
               << " n=" << n << " gg/gmax=" << xg << " prob=" << gg << " gmax=" << gmax << G4endl;
        G4cout << "    E=" << e << " Emin=" << fEmin << " Emax=" << fEmax << " E1=" << fE1
               << " E2=" << fE2 << " E3=" << E3 << " F0=" << Q0 << " F1=" << Q1 << " F2=" << Q2
               << " F3=" << Q3 << " F4=" << Q4 << G4endl;
      }
    }
