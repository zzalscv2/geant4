...
G4BiasingProcessInterface::G4BiasingProcessInterface(const G4String& name)
  : G4VProcess(name), fResetWrappedProcessInteractionLength(true)
{
  for (G4int i = 0; i < 8; i++)
...
G4BiasingProcessInterface::G4BiasingProcessInterface(G4VProcess* wrappedProcess,
                                                     ...
                                                     G4bool wrappedIsPostStep, G4String useThisName)
  : G4VProcess(useThisName != "" ? useThisName
    ...
    fWrappedProcessIsPost(wrappedIsPostStep)
{
  ...
  fPhysicalInteractionLaw =
    new G4InteractionLawPhysical("PhysicalInteractionLawFor(" + GetProcessName() + ")");
  // -- instantiate particle change wrapper for occurrence biaising:
  fOccurenceBiasingParticleChange =
    new G4ParticleChangeForOccurenceBiasing("biasingPCfor" + GetProcessName());
...
void G4BiasingProcessInterface::StartTracking(G4Track* track)
{
  ...
  if (fCommonStart.Get())
  {
    ...
    // -- Add a  "fSharedData->nStarting" here and outside bracket "fSharedData->nStarting++" and "
    // if (fSharedData->nStarting) == fSharedData->(vector interface length)"
...
{
  // ---------------------------------------------------------------------------------------------------
  // -- The "biasing process master" takes care of updating the biasing operator, and for all
  // biasing
  // -- processes it invokes the PostStepGPIL of physical wrapped processes (anticipate stepping
  // manager
  // -- call ! ) to make all cross-sections updated with current step, and hence available before
  // the
  ...
  if (fIamFirstGPIL)
  {
    ...
    if (fSharedData->fParallelGeometriesLimiterProcess)
    {
      ...
      for (auto wasLimiting : fSharedData->fParallelGeometriesLimiterProcess->GetWasLimiting())
      {
        if (firstStep || wasLimiting)
        {
          ...
          auto tmpParallelOperator = G4VBiasingOperator::GetBiasingOperator(
            (fSharedData->fParallelGeometriesLimiterProcess->GetCurrentVolumes()[iParallel])
              ->GetLogicalVolume());
          if (newParallelOperator)
          {
            if (tmpParallelOperator)
            {
              ...
              ed << " Keeping `" << newParallelOperator->GetName()
                 << "'. Behavior not guaranteed ! Please consider having only one operator at a "
                    "place."
                 << G4endl;
            ...
            newParallelOperator = tmpParallelOperator;
        }
        ++iParallel;
      }
    ...
    // -- [§§ Note : bug with this first step ? Does not work if previous step was concurrently
    // limited with geometry. Might make use of safety at last point ?]
    ...
    {
      G4VBiasingOperator* newOperator =
        G4VBiasingOperator::GetBiasingOperator(track.GetVolume()->GetLogicalVolume());
      fSharedData->fMassGeometryOperator = newOperator;
      if ((newOperator != nullptr) && (fSharedData->fParallelGeometryOperator != nullptr))
      {
        G4ExceptionDescription ed;
        ed << " Biasing operators are defined at the same place in mass and parallel geometries ! "
              "Found:\n";
        ed << "    - `" << fSharedData->fParallelGeometryOperator->GetName()
           << "' in parallel geometry and \n";
        ed << "    - `" << newOperator->GetName() << "' in mass geometry.\n";
        ed << " Keeping `" << fSharedData->fParallelGeometryOperator->GetName()
    ...
    // -- conclude the operator selection, giving priority to parallel geometry (as told in
    // exception message BIAS.GEN.30):
    ...
    if (fSharedData->fCurrentBiasingOperator != nullptr)
    {
      ...
      {
        (fSharedData->fPhysicsBiasingProcessInterfaces)[i]->InvokeWrappedProcessPostStepGPIL(
      ...
      || (fSharedData->fCurrentBiasingOperator != nullptr))
  {
    ...
    // fWrappedProcessInteractionLength : updated by InvokeWrappedProcessPostStepGPIL(...) above;
    // inverse of analog cross-section. fWrappedProcessForceCondition    : updated by
    // InvokeWrappedProcessPostStepGPIL(...) above
  ...
  if (fSharedData->fCurrentBiasingOperator == nullptr)
  {
    ...
    if (fIsPhysicsBasedBiasing)
    {
      return fWrappedProcess->PostStepGetPhysicalInteractionLength(track, usedPreviousStepSize,
                                                                   condition);
    }
    else
    {
  ...
  if (!fIsPhysicsBasedBiasing)
  {
    fNonPhysicsBiasingOperation =
      (fSharedData->fCurrentBiasingOperator)->GetProposedNonPhysicsBiasingOperation(&track, this);
    if (fNonPhysicsBiasingOperation == nullptr)
    {
      *condition = NotForced;
      return DBL_MAX;
    }
    return fNonPhysicsBiasingOperation->DistanceToApplyOperation(&track, previousStepSize,
                                                                 condition);
  }
  ...
  fOccurenceBiasingOperation =
    (fSharedData->fCurrentBiasingOperator)->GetProposedOccurenceBiasingOperation(&track, this);

  // -- no operation for occurrence biasing, analog GPIL returns the wrapped process GPIL and
  // condition values
  ...
  // -- 1) update process interaction length for reference analog interaction law (
  // fWrappedProcessInteractionLength updated/collected above):
  fPhysicalInteractionLaw->SetPhysicalCrossSection(1.0 / fWrappedProcessInteractionLength);
  // -- 2) Collect biasing interaction law:
  // --    The interaction law pointer is collected as a const pointer to the interaction law
  // object.
  // --    This interaction law will be kept under control of the biasing operation, which is the
  // only
  // --    entity that will change the state of the biasing interaction law.
  // --    The force condition for biasing is asked at the same time, passing the analog one as
  // default:
  fBiasingForceCondition = fWrappedProcessForceCondition;
  fBiasingInteractionLaw =
    fOccurenceBiasingOperation->ProvideOccurenceBiasingInteractionLaw(this, fBiasingForceCondition);
...
G4VParticleChange* G4BiasingProcessInterface::PostStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  if (!fIsPhysicsBasedBiasing)
  {
    G4VParticleChange* particleChange =
      fNonPhysicsBiasingOperation->GenerateBiasingFinalState(&track, &step);
    (fSharedData->fCurrentBiasingOperator)
      ->ReportOperationApplied(this, BAC_NonPhysics, fNonPhysicsBiasingOperation, particleChange);
  ...
  fFinalStateBiasingOperation =
    (fSharedData->fCurrentBiasingOperator)->GetProposedFinalStateBiasingOperation(&track, this);
  // -- Flag below is to force the biased generated particle change to be returned "as is" to the
  // stepping, disregarding there
  // -- was or not a occurrence biasing that would apply. Weight relevance under full responsibility
  // of the biasing operation.
  G4bool forceBiasedFinalState = false;
  if (fFinalStateBiasingOperation != nullptr)
  {
    finalStateParticleChange = fFinalStateBiasingOperation->ApplyFinalStateBiasing(
  ...
  {
    (fSharedData->fCurrentBiasingOperator)
      ->ReportOperationApplied(this, BAC, fFinalStateBiasingOperation, finalStateParticleChange);
  ...
  // -- If occurrence biasing, applies the occurrence biasing weight correction on top of final
  // state (biased or not):
  ...
  else
  {
    ...
    if (!fBiasingInteractionLaw->IsEffectiveCrossSectionInfinite())
    {
      ...
      G4Exception(" G4BiasingProcessInterface::PostStepDoIt(...)", "BIAS.GEN.02", JustWarning, ed);
  ...
  {
    G4ExceptionDescription ed;
    ed << " Negative interaction weight : w_I = " << weightForInteraction << " XS_I(phys) = "
       << fBiasingInteractionLaw->ComputeEffectiveCrossSectionAt(step.GetStepLength())
       << " XS_I(bias) = "
       << fPhysicalInteractionLaw->ComputeEffectiveCrossSectionAt(step.GetStepLength())
       << " step length = " << step.GetStepLength() << " Interaction law = `"
       << fBiasingInteractionLaw << "'" << G4endl;
    G4Exception(" G4BiasingProcessInterface::PostStepDoIt(...)", "BIAS.GEN.03", JustWarning, ed);
  }

  (fSharedData->fCurrentBiasingOperator)
    ->ReportOperationApplied(this, BAC, fOccurenceBiasingOperation, weightForInteraction,
  ...
  fOccurenceBiasingParticleChange
    ->StealSecondaries();  // -- this also makes weightForInteraction applied to secondaries stolen
...
// -- AlongStep methods:
G4double G4BiasingProcessInterface::AlongStepGetPhysicalInteractionLength(
  ...
  *selection = NotCandidateForSelection;
  ...
  if (fSharedData->fCurrentBiasingOperator == nullptr)
  {
    if (fWrappedProcessIsAlong)
      fWrappedProcessAlongStepGPIL = fWrappedProcess->AlongStepGetPhysicalInteractionLength(
  ...
  G4double minimumStep =
    fBiasingAlongStepGPIL < currentMinimumStep ? fBiasingAlongStepGPIL : currentMinimumStep;
  // -- wrapped process is called with minimum step ( <= currentMinimumStep passed ) : an along
  // process can not
  // -- have its operation stretched over what it expects:
  if (fWrappedProcessIsAlong)
  {
    fWrappedProcessAlongStepGPIL = fWrappedProcess->AlongStepGetPhysicalInteractionLength(
      track, previousStepSize, minimumStep, proposedSafety, selection);
    fWrappedProcessGPILSelection = *selection;
    fBiasingGPILSelection =
      fOccurenceBiasingOperation->ProposeGPILSelection(fWrappedProcessGPILSelection);
  }
  else
  {
    fBiasingGPILSelection =
      fOccurenceBiasingOperation->ProposeGPILSelection(NotCandidateForSelection);
                                                            ...
                                                            const G4Step& step)
{
  ...
  if (fWrappedProcessIsAlong)
  {
    fOccurenceBiasingParticleChange->SetWrappedParticleChange(
      fWrappedProcess->AlongStepDoIt(track, step));
  }
  else
  {
  ...
  if (fBiasingInteractionLaw != nullptr)
  {
    ...
    if (weightForNonInteraction <= 0.)
    {
      ...
      ed << " Negative non interaction weight : w_NI = " << weightForNonInteraction
         ...
         << " p_NI(bias) = "
         << fBiasingInteractionLaw->ComputeNonInteractionProbabilityAt(step.GetStepLength())
         << " step length = " << step.GetStepLength() << " biasing interaction law = `"
         << fBiasingInteractionLaw->GetName() << "'" << G4endl;
      G4Exception(" G4BiasingProcessInterface::AlongStepDoIt(...)", "BIAS.GEN.04", JustWarning, ed);
    }
  }
...
void G4BiasingProcessInterface::BuildPhysicsTable(const G4ParticleDefinition& pd)
{
  ...
  if (fIamFirstGPIL)
  {
    ...
    // -- The fDoCommonConfigure is to ensure that this Configure is made by only one process
    // (othewise each first process makes the call):
...
{
  if (fWrappedProcess != nullptr) fWrappedProcess->ResetNumberOfInteractionLengthLeft();
}
                                                                 ...
                                                                 G4ForceCondition* condition)
{
  ...
  // -- GPIL response:
  fWrappedProcessPostStepGPIL =
    fWrappedProcess->PostStepGetPhysicalInteractionLength(track, usedPreviousStepSize, condition);
