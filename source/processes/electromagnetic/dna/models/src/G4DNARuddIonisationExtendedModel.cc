...
namespace
{
G4Mutex ionRuddExtendedMutex = G4MUTEX_INITIALIZER;
...
const G4double Bj[5] = {12.60 * CLHEP::eV, 14.70 * CLHEP::eV, 18.40 * CLHEP::eV, 32.20 * CLHEP::eV,
                        539 * CLHEP::eV};
}  // namespace
  ...
  : G4VEmModel(nam)
{
  ...
  // low energy limit of proton table
  fLowestEnergy = 100 * CLHEP::eV;

  // low energy limit for alpha table
...
void G4DNARuddIonisationExtendedModel::LoadData()
{
  ...
  G4AutoLock l(ionRuddExtendedMutex);
  if (nullptr != xshelium)
  {
    l.unlock();
    return;
  ...
  fpWaterDensity = G4DNAMolecularMaterial::Instance()->GetNumMolPerVolTableFor(water);
  l.unlock();
}
                                                  ...
                                                  const G4DataVector&)
{
  ...
  if (!isInitialised)
  {
    ...
    // low energy tracking cut is taken from EM parameters
    G4EmParameters* param = G4EmParameters::Instance();
    fLowestEnergy = param->MinDNAProtonEnergy();
    G4double eminI = param->MinDNAIonEnergy();
    ...
    if (verbose > 0)
    {
      G4cout << "### G4DNARuddIonisationExtendedModel::Initialise(..) " << pname
             << "/n    idx=" << idx << " isIon=" << isIon << " isHelium=" << isHelium << G4endl;
    }
  }
}
...
G4double G4DNARuddIonisationExtendedModel::CrossSectionPerVolume(const G4Material* material,
                                                                 ...
                                                                 G4double kinE, G4double, G4double)
{
  // check if model is applicable for given material
  G4double density =
    (material->GetIndex() < fpWaterDensity->size()) ? (*fpWaterDensity)[material->GetIndex()] : 0.0;
  if (0.0 == density)
  {
    return 0.0;
  }
  ...
  G4double sigma = (e > fElow) ? xscurrent->FindValue(e) : xscurrent->FindValue(fElow) * e / fElow;
  ...
  if (verbose > 1)
  {
    G4cout << "G4DNARuddIonisationExtendedModel for " << part->GetParticleName()
           << " Ekin(keV)=" << kinE / CLHEP::keV << " sigma(cm^2)=" << sigma / CLHEP::cm2 << G4endl;
  }
  return sigma;
}
...
void G4DNARuddIonisationExtendedModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
                                                         ...
                                                         const G4DynamicParticle* dpart, G4double,
                                                         G4double)
{
  ...
  G4double bindingEnergy =
    (useDNAWaterStructure) ? waterStructure.IonisationEnergy(shell) : Bj[shell];

  // Si: additional protection if tcs interpolation method is modified
  if (kinE < bindingEnergy)
  {
    return;
  }
  ...
  if (scatteredEnergy < -tolerance || exc < -tolerance)
  {
    G4cout << "G4DNARuddIonisationExtendedModel::SampleSecondaries: "
           ...
           << " Ein(keV)=" << kinE / CLHEP::keV << "  " << pd->GetParticleName()
  ...
  */
  ...
  G4DNAChemistryManager::Instance()->CreateWaterMolecule(eIonizedMolecule, shell, theIncomingTrack);
}
...
G4double G4DNARuddIonisationExtendedModel::SampleElectronEnergy(G4double kine, G4int shell)
{
  ...
  for (G4int i = 0; i < 100000; ++i)
  {
    ...
    if (y > ymax && count < 10)
    {
      ...
      G4cout << "G4DNARuddIonisationExtendedModel::SampleElectronEnergy warning: "
             ...
             << " Edelta(keV)=" << deltae / CLHEP::keV << " y=" << y << " ymax=" << ymax
             << " n=" << i << G4endl;
    }
...
G4double G4DNARuddIonisationExtendedModel::ProbabilityFunction(G4double kine, G4double deltae,
                                                               G4int)
{
  ...
  G4double res = (F1 + w * F2) / (fGpow->powN((1. + w) / u, 3) * y);

  if (0 == idx)
  {
    res *= CorrectionFactor(kine);
  }
...
G4double G4DNARuddIonisationExtendedModel::S_1s(G4double kine, G4double energyTransfer,
                                                G4double slaterEffCharge, G4double shellNumber)
{
  // 1 - e^(-2r) * ( 1 + 2 r + 2 r^2)
...
G4double G4DNARuddIonisationExtendedModel::S_2s(G4double kine, G4double energyTransfer,
                                                G4double slaterEffCharge, G4double shellNumber)
{
...
G4double G4DNARuddIonisationExtendedModel::S_2p(G4double kine, G4double energyTransfer,
                                                G4double slaterEffCharge, G4double shellNumber)
{
...
G4double G4DNARuddIonisationExtendedModel::Rh(G4double ekin, G4double etrans, G4double q,
                                              G4double shell)
{
  // The following values are provided by M. Dingfelder (priv. comm)
...
G4double G4DNARuddIonisationExtendedModel::CorrectionFactor(const G4double kine)
{
  // Correction factor for Hydrogen only
  const G4double ln10 = fGpow->logZ(10);
  G4double x = 2.0 * ((G4Log(kine / CLHEP::eV) / ln10) - 4.2);
  // The following values are provided by M. Dingfelder (priv. comm)
  G4double res = 0.6 / (1.0 + G4Exp(x)) + 0.9;
  return res;
}
