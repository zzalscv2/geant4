    ...
    estep(maxEnergy / (G4double)(npti - 1))
{
  ...
  fResIon = const_cast<const G4ParticleDefinition*>(
    theIonTable->GetIon(daughterZ, daughterA, excitationE, flb));
...
void G4BetaMinusDecay::SetUpBetaSpectrumSampler(const G4int& daughterZ, const G4int& daughterA,
                                                const G4BetaDecayType& betaType)
{
  cdf[0] = 0.0;
...
void G4BetaMinusDecay::DumpNuclearInfo()
{
  G4cout << " G4BetaMinusDecay  " << fPrimaryIon->GetParticleName() << " -> "
         << fResIon->GetParticleName() << " + " << fLepton->GetParticleName() << " + "
         << fNeutrino->GetParticleName() << " Eemax(MeV)=" << maxEnergy * eMass << " BR=" << GetBR()
         << "%" << G4endl;
}
