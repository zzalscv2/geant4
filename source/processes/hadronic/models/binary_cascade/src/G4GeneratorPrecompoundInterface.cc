  ...
  : CaptureThreshold(70 * MeV), DeltaM(5.0 * MeV), DeltaR(0.0), secID(-1)
{
  ...
  if (preModel)
  {
    SetDeExcitation(preModel);
  }
  else
  {
    G4HadronicInteraction* hadi = G4HadronicInteractionRegistry::Instance()->FindModel("PRECO");
...
G4GeneratorPrecompoundInterface::Propagate(G4KineticTrackVector* theSecondaries,
                                           G4V3DNucleus* theNucleus)
{
  ...
  for (iter = theSecondaries->begin(); iter != theSecondaries->end(); ++iter)
  {
    ...
    if ((part != proton && part != neutron) || ((*iter)->GetPosition().mag() > R))
    {
  ...
  G4Nucleon* theCurrentNucleon = theNucleus->StartLoop() ? theNucleus->GetNextNucleon() : 0;
  ...
  G4cout << "Sum 4 momenta  " << Secondary4Momentum + captured4Momentum + Residual4Momentum
         << G4endl;
#endif
  ...
  while (theCurrentNucleon) /* Loop checking, 31.08.2015, G.Folger */
  {
    if (theCurrentNucleon->AreYouHit())
    {
      if (theCurrentNucleon->Get4Momentum().mag()
          < theCurrentNucleon->GetDefinition()->GetPDGMass())
        QGSM = true;
    }
    theCurrentNucleon = theNucleus->GetNextNucleon();
  }
  ...
  if (!QGSM)
  {
    ...
    if (numberOfEx == 0)
    {
      G4cout << "Residual  4Mom = 0 means that there were not wounded and captured nucleons"
             << G4endl;
    }
  }
#endif
  ...
  if (anA >= aZ)
  {
    ...
    {  // QGS model was used
      ...
      exciton4Momentum = GetPrimaryProjectile()->Get4Momentum()
                         + G4LorentzVector(0., 0., 0., InitialTargetMass) - Secondary4Momentum;
...
#ifdef debugPrecoInt
        G4cout << "ActualMass - fMass < 0. " << ActualMass << " " << fMass << " "
               << ActualMass - fMass << G4endl;
#endif
      }
    }
...
G4HadFinalState* G4GeneratorPrecompoundInterface::ApplyYourself(const G4HadProjectile&, G4Nucleus&)
{
...
G4ReactionProductVector*
G4GeneratorPrecompoundInterface::PropagateNuclNucl(G4KineticTrackVector* theSecondaries,
                                                   G4V3DNucleus* theNucleus,
                                                   G4V3DNucleus* theProjectileNucleus)
{
  ...
  G4bool ProjectileIsAntiNucleus = GetPrimaryProjectile()->GetDefinition()->GetBaryonNumber() < -1;
  ...
  G4int aLb = theProjectileNucleus
                ->GetNumberOfLambdas();  // Non negative number of (anti-)lambdas in (anti-)nucleus
  G4int numberOfExB = 0;
  G4int numberOfChB = 0;
  ...
  while (theCurrentNucleon) /* Loop checking, 31.08.2015, G.Folger */
  {
    if (theCurrentNucleon->AreYouHit())
    {
      ...
      {
        aZb -= G4int(theCurrentNucleon->GetDefinition()->GetPDGCharge() / eplus + 0.1);
  ...
  G4bool ExistTargetRemnant = G4double(numberOfHoles) < 0.3 * G4double(numberOfHoles + anA);
  G4bool ExistProjectileRemnant = G4double(numberOfHolesB) < 0.3 * G4double(numberOfHolesB + anAb);

#ifdef debugPrecoInt
  G4cout << "Projectile residual A Z (numberOfLambdas) E* 4mom " << anAb << " " << aZb << " ("
         << aLb << ") " << exEnergyB << " " << Projectile4Momentum << G4endl;
  G4cout << " ExistTargetRemnant ExistProjectileRemnant " << ExistTargetRemnant << " "
         << ExistProjectileRemnant << G4endl;
#endif
  //-----------------------------------------------------------------------------
  // decay the strong resonances
  G4ReactionProductVector* theTotalResult = new G4ReactionProductVector;
  ...
  for (iter = theSecondaries->begin(); iter != theSecondaries->end(); ++iter)
  {
    ...
    if (part != proton && part != neutron && (part != ANTIproton && ProjectileIsAntiNucleus)
        && (part != ANTIneutron && ProjectileIsAntiNucleus))
    {
      ...
      G4cout << "Secondary  " << SecondrNum << " " << theNew->GetDefinition()->GetParticleName()
    ...
    if (!ProjectileIsAntiNucleus && (part == proton || part == neutron))
    {
    ...
    if (ProjectileIsAntiNucleus && (part == ANTIproton || part == ANTIneutron))
    {
      CanBeCapturedByProjectile = ExistProjectileRemnant
                                  && (-CaptureThreshold * G4Log(G4UniformRand())
                                      > (aTrack4Momentum + Projectile4Momentum).mag()
    ...
    if (CanBeCapturedByTarget && CanBeCapturedByProjectile)
    {
      ...
      {
        CanBeCapturedByTarget = true;
        CanBeCapturedByProjectile = false;
      }
      else
      {
        CanBeCapturedByTarget = false;
        CanBeCapturedByProjectile = true;
      }
    }
...
*/
...
#ifdef debugPrecoInt
  G4cout << "Final target residual A Z (numberOfLambdas) E* 4mom " << anA << " " << aZ
         << " (0"  //<<aL
         << ") " << exEnergy << " " << Target4Momentum << G4endl;
#endif
  ...
  if (0 != anA)
  {
    ...
    if (RemnMass < fMass)
    {
      ...
      Target4Momentum.setE(std::sqrt(Target4Momentum.vect().mag2() + RemnMass * RemnMass));
    }
    else
    {
  ...
  if (0 != anAb)
  {
    ...
    if (RemnMass < fMass)
    {
      ...
      Projectile4Momentum.setE(std::sqrt(Projectile4Momentum.vect().mag2() + RemnMass * RemnMass));
    }
    else
    {
    ...
    for (unsigned int ll = 0; ll < aPrecoResult->size(); ++ll)
    {
      ...
      if (ProjectileIsAntiNucleus)
      {
        ...
        if (aFragment == proton)
        {
          LastFragment = G4AntiProton::AntiProtonDefinition();
        }
        else if (aFragment == neutron)
        {
          LastFragment = G4AntiNeutron::AntiNeutronDefinition();
        }
        else if (aFragment == lambda)
        {
          LastFragment = G4AntiLambda::AntiLambdaDefinition();
        }
        else if (aFragment == deuteron)
        {
          LastFragment = G4AntiDeuteron::AntiDeuteronDefinition();
        }
        else if (aFragment == triton)
        {
          LastFragment = G4AntiTriton::AntiTritonDefinition();
        }
        else if (aFragment == He3)
        {
          LastFragment = G4AntiHe3::AntiHe3Definition();
        }
        else if (aFragment == He4)
        {
          LastFragment = G4AntiAlpha::AntiAlphaDefinition();
        }
        else
        {}
...
void G4GeneratorPrecompoundInterface::MakeCoalescence(G4KineticTrackVector* tracks)
{
  ...
  {  // search for protons
    ...
    G4LorentzVector ProtSPposition =
      G4LorentzVector(trackP->GetPosition(), trackP->GetFormationTime());
    ...
    {  // search for neutron
      ...
      G4LorentzVector NeutSPposition =
        G4LorentzVector(trackN->GetPosition(), trackN->GetFormationTime() * hbarc / fermi);
      G4double EffMass = (Prot4Mom + Neut4Mom).mag();

      if (EffMass <= MassCut)
      {  // && (EffDistance <= SpaceCut)) { // Create deuteron
        G4KineticTrack* aDeuteron = new G4KineticTrack(
          deuteron, (trackP->GetFormationTime() + trackN->GetFormationTime()) / 2.0,
          (trackP->GetPosition() + trackN->GetPosition()) / 2.0, (Prot4Mom + Neut4Mom));
