...
const G4String fFloatingLevels[13] = {"-",  "+X", "+Y", "+Z", "+U", "+V", "+W",
                                      "+R", "+S", "+T", "+A", "+B", "+C"};
}  // namespace

G4LevelReader::G4LevelReader(G4NuclearLevelData* ptr) : fData(ptr)
{
...
const G4LevelManager* G4LevelReader::CreateLevelManager(G4int Z, G4int A)
{
  ...
  if (!infile.is_open())
  {
    if (fVerbose > 1)
    {
      ...
      ed << "Regular file " << ss.str() << " is not opened! Z=" << Z << " A=" << A;
      G4Exception("G4LevelReader::LevelManager(..)", "had014", JustWarning, ed, "Check file path");
    }
    return nullptr;
  }
  // file is opened
  if (fVerbose > 1)
  {
    G4cout << "G4LevelReader: open file " << ss.str() << " for Z= " << Z << " A= " << A << G4endl;
  }
  return LevelManager(Z, A, infile);
}

const G4LevelManager* G4LevelReader::MakeLevelManager(G4int Z, G4int A, const G4String& filename)
{
  ...
  if (!infile.is_open())
  {
    ...
    {
      G4ExceptionDescription ed;
      ed << "External file " << filename << " is not opened! Z=" << Z << " A=" << A;
      G4Exception("G4LevelReader::LevelManager(..)", "had014", FatalException, ed,
                  "Check file path");
    }
    return nullptr;
  }
  ...
  if (fVerbose > 1)
  {
    G4cout << "G4LevelReader: open external file " << filename << " for Z= " << Z << " A= " << A
           << G4endl;
  }
  return LevelManager(Z, A, infile);
}

const G4LevelManager* G4LevelReader::LevelManager(G4int Z, G4int A, std::ifstream& infile)
{
  ...
  for (;;)
  {
    ...
    if (infile.eof())
    {
      if (fVerbose > 1)
      {
        G4cout << "### End of file Z= " << Z << " A= " << A << " Nlevels= " << i << G4endl;
      }
      break;
    }
...
#endif
    // read new level data
    if (!(ReadDataItem(infile, fEnergy) && ReadDataItem(infile, fTime)
          && ReadDataItem(infile, fSpin) && ReadDataItem(infile, ntrans)))
    {
      if (fVerbose > 1)
      {
        G4cout << "### Incomplete end of file Z= " << Z << " A= " << A << " Nlevels= " << i
               << G4endl;
      }
      break;
    }
    ...
    if (0 < i)
    {
      if (fEnergy < vEnergy[i - 1])
      {
        ...
        {
          G4cout << "### G4LevelReader: broken level " << i << " E(MeV)= " << fEnergy << " < "
                 << vEnergy[i - 1] << " for isotope Z= " << Z << " A= " << A
                 << " level energy increased" << G4endl;
        }
#endif
        // for any case
    ...
    if (fVerbose > 2)
    {
      G4cout << "   Level #" << i1 << " E(MeV)=" << fEnergy / CLHEP::MeV << "  LTime(s)=" << fTime
             << " 2S=" << vSpin[i] << "  meta=" << vSpin[i] / 100000 << " idx=" << i
             << " ntr=" << ntrans << G4endl;
    }
#endif
    vLevel[i] = nullptr;
    if (ntrans == 0)
    {
      vLevel[i] = new G4NucLevel(0, fTime, vTrans, vGammaCumProbability, vGammaProbability, vRatio,
                                 vShellProbability);
    }
    else if (ntrans > 0)
    {
      ...
      {
        if (!(ReadDataItem(infile, i2) && ReadDataItem(infile, fTransEnergy)
              && ReadDataItem(infile, fProb) && ReadDataItem(infile, tnum)
              && ReadDataItem(infile, vRatio[j]) && ReadDataItem(infile, fAlpha)))
        {
#ifdef G4VERBOSE
          ++count2;
          if (count2 < countmax && fVerbose > 0)
          {
            G4cout << "### Fail to read transition j=" << j << " j=" << j << " i2=" << i2
                   << " Z=" << Z << " A=" << A << G4endl;
          }
#endif
          isTransOK = false;
        ...
        if (i2 >= i)
        {
          ...
          if (count2 < countmax)
          {
            G4cout << "### G4LevelReader: broken transition " << j << " from level " << i << " to "
                   << i2 << " for isotope Z= " << Z << " A= " << A
                   << "; the transition probability set to zero" << G4endl;
          }
#endif
          isTransOK = false;
          fProb = 0.0f;
        ...
        if (fVerbose > 2)
        {
          ...
          G4cout << "### Transition #" << j << " to level " << i2 << " i2= " << i2
                 << " Etrans(MeV)= " << fTransEnergy * CLHEP::keV << "  fProb= " << fProb
                 << " MultiP= " << tnum << "  fMpRatio= " << fRatio << " fAlpha= " << fAlpha
                 << G4endl;
          G4cout.precision(prec);
        }
        if (fAlpha > 0.0f)
            ...
            if (!ReadDataItem(infile, fICC[k]))
            {
              ...
              if (count2 < countmax)
              {
                G4cout << "### G4LevelReader: fail to read conversion coeff k= " << k
                       << " for transition j= " << j << "  Z= " << Z << " A= " << A << G4endl;
              }
#endif
              for (kk = k; kk < 10; ++kk)
              {
      ...
      if (ntrans > 0)
      {
        ...
        if (fVerbose > 2)
        {
          G4cout << "=== New G4NucLevel: Ntrans=" << ntrans << " Time(ns)=" << fTime
                 << " IdxTrans=" << vTrans[nt] / 10000 << " isOK=" << isTransOK << G4endl;
        }
        ...
        for (k = 0; k < nt; ++k)
        {
          ...
          if (fVerbose > 3)
          {
            G4cout << "Probabilities[" << k << "]= " << vGammaCumProbability[k] << "  "
                   << vGammaProbability[k] << " idxTrans= " << vTrans[k] / 10000 << G4endl;
          }
#endif
        }
        vGammaCumProbability[nt] = 1.0f;
        vLevel[i] = new G4NucLevel((std::size_t)ntrans, fTime, vTrans, vGammaCumProbability,
                                   vGammaProbability, vRatio, vShellProbability);
      }
    }
    ++i;
    if (i == fLevelMax)
  ...
  if (1 <= i)
  {
    ...
    if (fVerbose > 1)
    {
      G4cout << "=== Reader: new manager for Z=" << Z << " A=" << A << " Nlevels=" << i
             << " E[0]=" << vEnergy[0] / CLHEP::MeV << " MeV   E1=" << vEnergy[i - 1] / CLHEP::MeV
             << " MeV"
